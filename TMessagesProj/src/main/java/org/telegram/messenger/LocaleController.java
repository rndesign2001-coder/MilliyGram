...
public class LocaleController {
    ...
    private String getStringInternal(String key, String fallback, int fallbackRes, int res) {
        String value = BuildVars.USE_CLOUD_STRINGS && !mgIsLocalOnly(key, res) ? localizationExternal.getByResNameOrResId(ApplicationLoader.applicationContext, key, res) : null;
        if (value == null) {
    ...
    private static boolean mgIsLocalOnly(String key, int res) {
        if (res != 0 && (res == R.string.AppName || res == R.string.AppNameBeta || res == R.string.TelegramVersion)) {
            return true;
        }
        return "AppName".equals(key) || "AppNameBeta".equals(key) || "TelegramVersion".equals(key) || (key != null && key.startsWith("MG_"));
    }
    ...
    private static String formatString(String key, String fallback, int res, int fallbackRes, Object... args) {
        try {
            String value = BuildVars.USE_CLOUD_STRINGS && !mgIsLocalOnly(key, res) ? getInstance().localizationExternal.getByResNameOrResId(ApplicationLoader.applicationContext, key, res) : null;
            if (value == null) {
                if (BuildVars.USE_CLOUD_STRINGS && fallback != null) {
